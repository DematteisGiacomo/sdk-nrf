...
/* Functions moved to lte_ip_addr_helper.c */
...
static int set_ntn_active_mode()
{
	...
	err = nrf_modem_at_printf("AT%%XBANDLOCK=2,,\"%i\"", CONFIG_NRF_MODEM_LIB_NET_IF_NTN_BANDLOCK);
	if (err) {
	...
	err = nrf_modem_at_printf("AT%%CHSELECT=1,14,%i", CONFIG_NRF_MODEM_LIB_NET_IF_NTN_CHANNEL_SELECT);
	if (err) {
...
int lte_net_if_enable_ntn(void)
{
	...
	if (!nrf_modem_is_initialized()) {
		...
		if (err) {
			LOG_ERR("Failed to set initialize nrf_modem_lib, error: %d", err);
